...
import { AcaiBowl } from '../components/Acai_bowl'
import { Spoon } from '../components/Spoon'
import { DryFlower } from '../components/Dry_flower'
import {
  Environment,
  Lightformer,
  RandomizedLight,
...
export default function App() {
  ...
  return (
    ...
    <Canvas
      shadows
      dpr={[1, perfSucks ? 1.5 : 20]}
      eventSource={document.getElementById('root')}
      eventPrefix="client"
      camera={{ position: [1, -5, 20],  far: 600  }}>
      {/** PerfMon will detect performance issues */}
      <PerformanceMonitor onDecline={() => degrade(true)} />
      <ambientLight intensity={0.2} />
      <directionalLight position={[0, 1, 0]} />
      <color attach="background" args={['#f0f0f0']} />
      <group position={[0.8, -1, -3]} rotation={[0.3, -0.5, 0]}>
        <AcaiBowl scale={2.5}/>
      </group>
      <group position={[-3, -3, 1]} rotation={[0.3, -0.5, 0]}>
        <Spoon scale={45}/>
      </group>
      <group position={[1, 0, -14]} rotation={[-0.3, -0.4, -1.7]}>
        <DryFlower scale={80} />
      </group>
...
function Env({ perfSucks }) {
  ...
  return (
    <Environment frames={perfSucks ? 1 : Infinity} preset="city" resolution={256} background blur={0.8}>
      ...
      <group ref={ref}>
        <Lightformer intensity={5} form="ring" color="red" rotation-y={Math.PI / 2} position={[-5, 2, -5]} scale={[50, 10, 10]} />
      </group>
    </Environment>
  )
}
